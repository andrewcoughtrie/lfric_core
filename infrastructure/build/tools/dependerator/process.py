...
class FortranProcessor():
    ...
    # Constructor.
    #
    def __init__(self, database: FortranDependencies,
    ...
    def determine_compile_file_dependencies(self,
                                            file_store: FileDependencies,
                                            object_modules=False):
        ...
        if not object_modules:
            if self.__module_directory is None:
                raise Exception("Cannot determine compile dependencies when "
            ...
            for module, file_path in self.__database.get_modules():
                ...
                module_path = self.__module_directory \
                    / file_path.parent / module_leafname

                source_object_path = self.__object_directory \
                    / file_path.with_suffix('.o')
        ...
        for unit, unit_path, unit_type, prereq, prereq_path, prereq_type in \
            ...
            prereq_object_path = self.__object_directory \
                / prereq_path.with_suffix('.o')
            ...
            if prereq_type == 'module':
                ...
                if object_modules:
                    file_store.add_file_dependency(unit_object_path,
                                                   prereq_object_path)
                    ...
                    prereq_module_path = self.__module_directory \
                        / prereq_path.parent / prereq_leaf
    ...
    #
    def determine_link_dependencies(self, root_unit: Optional[str] = None) \
        ...
        for root in roots:
            ...
            for unit, unit_file, prereq, prereq_file \
                ...
                objects.add(self.__object_directory
                            / prereq_file.with_suffix('.o'))

            if root_object_file is None:
                raise Exception(f"Root object '{root}' not found")

            yield self.__object_directory / root, \
                root_object_file, \
                sorted(objects)
