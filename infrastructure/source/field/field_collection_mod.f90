...
module field_collection_mod
  ...
  contains
    procedure, public :: next => nextr
    procedure, public :: has_next => has_nextr
...
function field_collection_real_iterator_constructor(collection) result(self)
  ...
  do
    ...
    ! Make sure first field pointed to in lst is a real field
...
!> @return A polymorphic field pointer to either a field or an integer field
...
!> @return A field pointer to the next (real) field in the collection
function nextr(self) result (field)
...
!> @return next Returns true if there is another field in the collection,
!>              and  false if there isn't.
function has_nextr(self) result(next)
