...
contains
  ...
  subroutine initialise( this,             &
    ...
    if (seconds_per_step <= 0.0_r_second) then
      ...
      this%seconds_per_step = seconds_per_step
    end if

    this%current_step = this%first_step
    this%last_spinup_step = ceiling( spinup_period / seconds_per_step )

    this%spinup_fraction = this%calculate_spinup_fraction()

    this%initialisation_phase = (this%current_step == 1_i_timestep)
    this%starting = .true.
  ...
  function get_calendar( this )
    ...
    class(clock_type), intent(in), target :: this
  ...
  !>
  !> While in the spinn up period calculate_spinup_fraction() may be used.
  !>
