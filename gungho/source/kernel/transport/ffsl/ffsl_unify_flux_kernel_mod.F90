...
contains
  ...
  !> @param[in]     ndf_w2        Number of degrees of freedom for a Cell in the W2 space
  !> @param[in]     undf_w2       Total number of degrees of freedom for the W2 space
  !> @param[in]     map_w2        Cell dofmap for the W2 space
  ...
  subroutine ffsl_unify_flux_kernel_code( nlayers,       &
    ...
    ! Arguments: Fields
    real(kind=r_tran), dimension(undf_w2),  intent(inout) :: flux_x, flux_y
      ...
      case(1)
        do df = 1, 4
          ...
          select case(halo)
            case(2)
              do k = 0, nlayers-1
                flux_x(map_w2(3)+k) = 0.5_r_def*(flux_x_broken(map_w2b(3)+k) &
                                               + flux_x_broken(smap_x(1,2,3)+k))
              end do
            case(4)
              do k = 0, nlayers-1
                flux_x(map_w2(1)+k) = 0.5_r_def*(flux_x_broken(map_w2b(1)+k) &
                                               - flux_y_broken(smap_y(2,2,1)+k))
              end do
            case(5)
              do k = 0, nlayers-1
                flux_y(map_w2(4)+k) = 0.5_r_def*(flux_y_broken(map_w2b(4)+k) &
                                               + flux_y_broken(smap_x(2,2,4)+k))
              end do
            case(6)
              do k = 0, nlayers-1
                flux_y(map_w2(2)+k) = 0.5_r_def*(flux_y_broken(map_w2b(2)+k) &
