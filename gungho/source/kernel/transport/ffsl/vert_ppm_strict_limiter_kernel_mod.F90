...
!> @brief Computes the fractional vertical mass flux using the PPM reconstruction with strict
!!        monotonicity.
...
!!          values at cell edges. Strict monotonicity is applied to the edge values and
...
module vert_ppm_strict_limiter_kernel_mod
...
type, public, extends(kernel_type) :: vert_ppm_strict_limiter_kernel_type
  private
...
contains
  procedure, nopass :: vert_ppm_strict_limiter_code
end type

!-------------------------------------------------------------------------------
! Contained functions/subroutines
!-------------------------------------------------------------------------------
public :: vert_ppm_strict_limiter_code
...
subroutine vert_ppm_strict_limiter_code( nlayers,   &
                                         flux_high, &
                                         frac_wind, &
                                         dep_pts,   &
                                         field,     &
  ...
  use subgrid_rho_mod, only: vertical_ppm_strict, &
                             fourth_order_vertical_edge_strict
  ...
  ! field_local and dz_local have index: | 1 | 2 | 3 | 4 | for fourth_order_vertical_edge_strict
  ...
  edge_to_do = 0_i_def
  call fourth_order_vertical_edge_strict( field_local, dz_local, edge_to_do, log_space, edge_below(k) )
  edge_to_do = 1_i_def
  call fourth_order_vertical_edge_strict( field_local, dz_local, edge_to_do, log_space, edge_below(k+1) )
  ...
  do k=0,nlayers-2
    ...
    ! Strict limited PPM reconstruction
    call vertical_ppm_strict(reconstruction, fractional_distance, field_1d(nc), &
...
end subroutine vert_ppm_strict_limiter_code

end module vert_ppm_strict_limiter_kernel_mod
