...
!!        rho, including monotonicity, in the vertical direction.
...
!!          with strict constraints applied to the coefficients to ensure monotonicity.
!!
...
!> @brief Compute the monotonic PPM subgrid reconstruction coefficients for a field.
...
subroutine vert_ppm_strict_limiter_code( nlayers,  &
  ...
  ! Compute PPM coefficients using strict limiter
  ...
  edge_to_do = 0_i_def
  call fourth_order_vertical_edge_strict( rho_local, dz_local, edge_to_do, edge_below(k) )
  edge_to_do = 1_i_def
  call fourth_order_vertical_edge_strict( rho_local, dz_local, edge_to_do, edge_below(k+1) )
