...
!> @brief Computes the fractional vertical mass flux using the monotonic Nirvana reconstruction.
...
!!          the gradient of the field at cell edges. A limiter is applied to ensure
...
module vert_nirvana_strict_kernel_mod
...
type, public, extends(kernel_type) :: vert_nirvana_strict_kernel_type
  private
...
contains
  procedure, nopass :: vert_nirvana_strict_code
end type

!-------------------------------------------------------------------------------
! Contained functions/subroutines
!-------------------------------------------------------------------------------
public :: vert_nirvana_strict_code

contains

!> @brief Compute the flux using the monotonic Nirvana reconstruction.
...
subroutine vert_nirvana_strict_code( nlayers,   &
                                     flux_high, &
                                     frac_wind, &
                                     dep_pts,   &
                                     field,     &
  ...
  do k=0,nlayers-2
    ...
    call vertical_nirvana_recon_strict(reconstruction, fractional_distance, field_stencil, &
...
end subroutine vert_nirvana_strict_code

end module vert_nirvana_strict_kernel_mod
