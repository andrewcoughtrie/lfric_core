...
contains
  ...
  function minmod_function(a,b) result(f)
    ...
    real(kind=r_tran), intent(in)    :: b
    real(kind=r_tran)                :: f
    if ( (a*b) <= 0.0_r_tran ) then
      f = 0.0_r_tran
    else
      if (abs(a) < abs(b)) then
        f = a
  ...
  subroutine second_order_coeffs(density,coeffs,positive,monotone)
    ...
    real(kind=r_tran)               :: cell_widths(1:5)
    real(kind=r_tran)               :: density_cell_edge_left
    real(kind=r_tran)               :: density_cell_edge_right

    coeffs(:) = 0.0_r_tran
    cell_widths(:) = (/ 1.0_r_tran,1.0_r_tran,1.0_r_tran,1.0_r_tran,1.0_r_tran /)
  ...
  function calc_density_at_cell_edge(cell_widths,density,positive,monotone) result(density_at_edge)
    ...
    real(kind=r_tran), intent(in) :: cell_widths(1:4)
    real(kind=r_tran), intent(in) :: density(1:4)
    logical, intent(in)           :: positive
    logical, intent(in)           :: monotone

    real(kind=r_tran) :: density_at_edge
    real(kind=r_tran) :: mass(1:4)
    real(kind=r_tran) :: y1,y2,y3,y4,y
    real(kind=r_tran) :: m1,m2,m3,m4
...
!     density_at_edge = (7.0_r_tran/12.0_r_tran)*(density(2)+density(3))-(1.0_r_tran/12.0_r_tran)*(density(1)+density(4))
