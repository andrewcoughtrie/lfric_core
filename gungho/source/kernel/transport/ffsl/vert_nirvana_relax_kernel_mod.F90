...
!> @brief Computes the fractional vertical mass flux using the relaxed monotonic Nirvana reconstruction.
...
!!          the gradient of the field at cell edges. A relaxed limiter is applied to ensure
!!          monotonicity. This kernel is designed to work in the vertical direction
!!          only and takes into account the vertical boundaries and grid spacing.
!!
...
module vert_nirvana_relax_kernel_mod
...
type, public, extends(kernel_type) :: vert_nirvana_relax_kernel_type
  private
...
contains
  procedure, nopass :: vert_nirvana_relax_code
end type

!-------------------------------------------------------------------------------
! Contained functions/subroutines
!-------------------------------------------------------------------------------
public :: vert_nirvana_relax_code

contains

!> @brief Compute the flux using the relaxed monotonic Nirvana reconstruction.
...
subroutine vert_nirvana_relax_code( nlayers,   &
                                    flux_high, &
                                    frac_wind, &
                                    dep_pts,   &
                                    field,     &
  ...
  integer(kind=i_def) :: df, nz, np, nm, nc
  ...
  real(kind=r_tran)   :: field_stencil(3)
  ...
  do k=0,nlayers-2
    ...
    ! Get index of neighbouring cells
    np = min(nc+1,nz-1)
    nm = max(nc-1,0)
    ! Vertical stencil needed for monotonicity
    field_stencil(1) = field_1d(nm)
    field_stencil(2) = field_1d(nc)
    field_stencil(3) = field_1d(np)

    ! Nirvana reconstruction
    call vertical_nirvana_recon_relax(reconstruction, fractional_distance, field_stencil, &
...
end subroutine vert_nirvana_relax_code

end module vert_nirvana_relax_kernel_mod
