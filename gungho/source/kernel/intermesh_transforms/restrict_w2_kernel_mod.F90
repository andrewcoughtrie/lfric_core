...
contains
  ...
  subroutine restrict_w2_code_r_single(nlayers,         &
    ...
    ! Fields
    real(kind=r_single), dimension(undf_c), intent(inout) :: coarse_field
    real(kind=r_single), dimension(undf_f), intent(in)    :: fine_field
    ...
    real(kind=r_single)    :: coarse_N, coarse_S, coarse_E, coarse_W, coarse_B
    ...
    do k = 0, nlayers-1
      ...
      coarse_N = 0.0_r_single
      coarse_S = 0.0_r_single

      do lp_x = 1, ncell_f_per_c_x
        ! N edge is first row of cell map
        lp_y = 1
        df = N
        coarse_N = coarse_N + ( real( rmultiplicity(dfmap_f(df,cell_map(lp_x,lp_y))+k), r_single) &
                               * fine_field(dfmap_f(df,cell_map(lp_x,lp_y))+k) )

        ! S edge is last row of cell map
        lp_y = ncell_f_per_c_y
        df = S
        coarse_S = coarse_S + ( real( rmultiplicity(dfmap_f(df,cell_map(lp_x,lp_y))+k), r_single) &
                               * fine_field(dfmap_f(df,cell_map(lp_x,lp_y))+k) )
      end do
      ...
      coarse_E = 0.0_r_single
      coarse_W = 0.0_r_single

      do lp_y = 1, ncell_f_per_c_y
        ! W edge is first column of cell map
        lp_x = 1
        df = W
        coarse_W = coarse_W + ( real( rmultiplicity(dfmap_f(df,cell_map(lp_x,lp_y))+k), r_single) &
                               * fine_field(dfmap_f(df,cell_map(lp_x,lp_y))+k) )

        ! E edge is last column of cell map
        lp_x = ncell_f_per_c_x
        df = E
        coarse_E = coarse_E + ( real( rmultiplicity(dfmap_f(df,cell_map(lp_x,lp_y))+k), r_single ) &
                               * fine_field(dfmap_f(df,cell_map(lp_x,lp_y))+k) )
      end do
  ...
  subroutine restrict_w2_code_r_double(nlayers,         &
    ...
    ! Fields
    real(kind=r_double), dimension(undf_c), intent(inout) :: coarse_field
    real(kind=r_double), dimension(undf_f), intent(in)    :: fine_field
    real(kind=r_def),    dimension(undf_f), intent(in)    :: rmultiplicity
    ...
    ! Internal variables
    integer(kind=i_def) :: df, k, lp_x, lp_y
    real(kind=r_double)    :: coarse_N, coarse_S, coarse_E, coarse_W, coarse_B
    ...
    do k = 0, nlayers-1
      ...
      coarse_N = 0.0_r_double
      coarse_S = 0.0_r_double

      do lp_x = 1, ncell_f_per_c_x
        ! N edge is first row of cell map
        lp_y = 1
        df = N
        coarse_N = coarse_N + ( real( rmultiplicity(dfmap_f(df,cell_map(lp_x,lp_y))+k), r_double) &
                               * fine_field(dfmap_f(df,cell_map(lp_x,lp_y))+k) )

        ! S edge is last row of cell map
        lp_y = ncell_f_per_c_y
        df = S
        coarse_S = coarse_S + ( real( rmultiplicity(dfmap_f(df,cell_map(lp_x,lp_y))+k), r_double) &
                               * fine_field(dfmap_f(df,cell_map(lp_x,lp_y))+k) )
      end do
      ...
      coarse_E = 0.0_r_double
      coarse_W = 0.0_r_double

      do lp_y = 1, ncell_f_per_c_y
        ! W edge is first column of cell map
        lp_x = 1
        df = W
        coarse_W = coarse_W + ( real( rmultiplicity(dfmap_f(df,cell_map(lp_x,lp_y))+k), r_double) &
                               * fine_field(dfmap_f(df,cell_map(lp_x,lp_y))+k) )

        ! E edge is last column of cell map
        lp_x = ncell_f_per_c_x
        df = E
        coarse_E = coarse_E + ( real( rmultiplicity(dfmap_f(df,cell_map(lp_x,lp_y))+k), r_double ) &
                               * fine_field(dfmap_f(df,cell_map(lp_x,lp_y))+k) )
      end do
